  ...
  const updateAnim = useCallback(() => {
    ...
    requestAnimationFrame(() => {
      [...dyAnims, ...dxAnims].forEach((anim) => {
        if (anim instanceof SVGAnimateElement && typeof anim.beginElement === "function") {
          try {
            anim.beginElement();
          } catch (err) {
            console.warn("Failed to begin SVG animation:", err);
          }
        }
      });
    });
  }, [filterId, speed, chaos, svgRef, rootRef, strokeRef]);
  useEffect(() => {
    updateAnim();
